...
async function loadAllContext() {
  ...
  const [
    ...
    masteryRuns,
    masteryNotes,
    // Built agents (what JARVIS has learned)
    builtAgents,
  ] = await Promise.all([
    ...
    db.from('shifts').select('date, duration_hours, calculated_pay').order('date', { ascending: false }).limit(20),
    ...
    db.from('mastery_runs').select('topic, status, started_at, completed_at').order('started_at', { ascending: false }).limit(20),
    db.from('mastery_notes').select('topic, note, created_at').order('created_at', { ascending: false }).limit(30),
    db.from('agents').select('topic, mastery_score, knowledge_summary, concepts, created_at').order('mastery_score', { ascending: false }).limit(20),
  ]);
  ...
  return {
    ...
    mastery: {
      runs:  masteryRuns.data  || [],
      notes: masteryNotes.data || [],
      agents: builtAgents.data || [],
    },
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { profile, learnings, sessions, lastHistory, health, productivity, finance, mastery } = ctx;
    ...
    // Mastery
    const masteryTopics = [...new Set(mastery.runs.map((r: any) => r.topic).filter(Boolean))];
    const masterySummary = masteryTopics.length > 0
      ? `Topics studied: ${masteryTopics.join(', ')}. ${mastery.runs.filter((r:any)=>r.status==='completed').length} sessions completed.`
      : 'No mastery sessions yet.';
    const masteryNotesSummary = mastery.notes.slice(0, 10)
      .map((n: any) => `[${n.topic}] ${n.note}`)
      .join('\n');

    // Built agents — what JARVIS has fully learned
    const agentsSummary = mastery.agents.length > 0
      ? mastery.agents.map((a: any) =>
          `• ${a.topic} (${a.mastery_score}% mastery) — ${a.knowledge_summary?.slice(0, 200) || 'no summary'}`
        ).join('\n')
      : 'No agents built yet.';
...
═══ JARVIS KNOWLEDGE BASE (built agents) ═══
${agentsSummary}

═══ MASTERY SESSIONS ═══
${masterySummary}
${masteryNotesSummary ? `Recent notes:\n${masteryNotesSummary}` : ''}
...
Synthesize everything above into an updated profile. Look for cross-domain patterns — does he log water on workout days but not rest days? Does he complete more todos when he sleeps better? These cross-table insights are the most valuable thing you can surface.`;
    ...
    try {
      ...
      synthesized = {
        ...
        synthesis_summary: rawResponse.slice(0, 500),
    ...
    // SMS notification if significant new insight found
    let smsText = '';
