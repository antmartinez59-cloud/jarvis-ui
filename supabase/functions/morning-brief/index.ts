...
async function getWeather(): Promise<any> {
  try {
    ...
    const data = await res.json();
    if (!data.ok || !data.weather) return null;
    const w = data.weather;
    // Build summary string that morning-brief builders expect
    w.summary = `${Math.round(w.temp)}°F, ${w.condition}. Feels like ${Math.round(w.feels_like)}°F. High ${Math.round(w.high)}°F / Low ${Math.round(w.low)}°F. Humidity ${w.humidity}%. Wind ${w.wind_mph}mph.`;
    return w;
  } catch { return null; }
}
...
async function getNewsData(sections: string[]): Promise<any> {
  try {
    ...
    const data = await res.json();
    // news function returns flat articles array — categorize for morning-brief
    const articles = data.articles || [];
    const financeKeywords = /market|stock|fed|economy|inflation|nasdaq|dow|s&p|gdp|trade|finance|crypto|bitcoin/i;
    const sportsKeywords  = /nfl|nba|mlb|nhl|soccer|sport|game|score|player|team|coach|league|espn/i;
    const techKeywords    = /tech|ai|apple|google|microsoft|meta|amazon|openai|software|startup|app/i;
    return {
      headlines: articles.filter((a: any) => !financeKeywords.test(a.title) && !sportsKeywords.test(a.title)),
      finance:   articles.filter((a: any) => financeKeywords.test(a.title)),
      sports:    articles.filter((a: any) => sportsKeywords.test(a.title)),
      tech:      articles.filter((a: any) => techKeywords.test(a.title)),
      random_fact: null,
      wordle_link: 'https://www.nytimes.com/games/wordle/index.html',
    };
  } catch { return { headlines:[], finance:[], sports:[], tech:[], random_fact:null, wordle_link:'https://www.nytimes.com/games/wordle/index.html' }; }
}
...
async function buildAfternoon(ctx: any, weather: any, news: any): Promise<{ subject: string; html: string; sms: string }> {
  ...
  const eveningWeather = weather?.forecast?.[0]
    ? `Tonight: ${weather.forecast[0].temp}°F, ${weather.forecast[0].condition}.`
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Save briefing to DB — only columns that exist in schema
    // Strip HTML so content is readable plain text in the dashboard
    const plainContent = result.sms || result.html.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
    const { data: savedBrief, error: insertErr } = await db.from('briefings').insert({
      briefing_type: briefingType,
      content:       plainContent,
    }).select().single();
    if (insertErr) console.error('[brief] DB insert failed:', insertErr.message);
